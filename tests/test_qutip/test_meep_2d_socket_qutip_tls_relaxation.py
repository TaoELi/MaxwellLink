...
qutip = pytest.importorskip("qutip", reason="qutip is required for this test")
...
def test_2d_qutip_tls_relaxation_matches_analytical_via_socket(plotting=False):
    """
    End-to-end (socket) QuTiP TLS relaxation test.

    Starts the external mxl_driver (QuTiP TLS model), runs a Meep simulation that
    ...
    try:
        ...
        if mp.am_master():
            ...
            driver_argv = _resolve_driver_path() + shlex.split(
                ...
                f'--param "preset=custom, module={usr_module_path}, fd_dmudt=false, kwargs=omega=0.242,mu12=187,orientation=2,pe_initial=1e-4,gamma_relax={gamma_relax}" '
            )
        ...
        if mp.am_master():
            ...
            gamma_relax_meep_units = gamma_relax / 0.242
            gamma = dipole_moment**2 * (frequency) ** 2 / 2.0 + gamma_relax_meep_units
            print("gamma", gamma)
...
if __name__ == "__main__":
    test_2d_qutip_tls_relaxation_matches_analytical_via_socket(plotting=True)
