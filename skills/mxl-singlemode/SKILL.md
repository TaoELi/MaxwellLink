...
- Thermal knobs: `temperature_au` (a.u., default `0.0`) seeds initial `qc`/`pc` from a Maxwell-Boltzmann distribution when positive, overriding any user-provided `qc_initial`/`pc_initial`. Add `langevin_tau_au` (Langevin relaxation time, a.u.) to also thermostat the cavity momentum every step; leave `langevin_tau_au=None` for a one-shot thermal initial condition with NVE dynamics. Use `random_seed` for reproducible sampling/kicks.
