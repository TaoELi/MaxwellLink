...
- Thermal initialization: when `temperature_au > 0`, `qc_initial`/`pc_initial` are overridden by Maxwell-Boltzmann samples at `temperature_au` (Hartree). Pair with `tau_au` for a Langevin thermostat on the cavity momentum; use `random_seed` for reproducibility. Convert temperatures with `from maxwelllink.units import AU_TO_K` (e.g. `T_au = 300.0 / AU_TO_K`).
